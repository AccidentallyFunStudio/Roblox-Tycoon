...
function BiomeService:UpgradeBiome(player : Player, biomeId : string)
    ...
    -- Verify ownership and that biome is not already at MaxLevel
    if not biomeState or biomeState.Level == 0 then return false end
    if biomeState.Level >= biomeConfig.MaxLevel then return false end

    -- Determine cost for the NEXT level
